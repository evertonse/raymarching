...
//  @modified
static SVec3		MikkNormalize( const SVec3 v )
{
...
static STSpace AvgTSpace(const STSpace * pTS0, const STSpace * pTS1)
{
	...
	else
	{
		...
		if ( VNotZero(ts_res.vOs) ) ts_res.vOs = MikkNormalize(ts_res.vOs);
		if ( VNotZero(ts_res.vOt) ) ts_res.vOt = MikkNormalize(ts_res.vOt);
	}
                             ...
                             const SMikkTSpaceContext * pContext)
{
	...
	for (g=0; g<iNrActiveGroups; g++)
	{
		...
		for (i=0; i<pGroup->iNrFaces; i++)	// triangles
		{
			...
			if ( VNotZero(vOs) ) vOs = MikkNormalize(vOs);
			if ( VNotZero(vOt) ) vOt = MikkNormalize(vOt);
			...
			for (j=0; j<pGroup->iNrFaces; j++)
			{
				...
				if ( VNotZero(vOs2) ) vOs2 = MikkNormalize(vOs2);
				if ( VNotZero(vOt2) ) vOt2 = MikkNormalize(vOt2);
                          ...
                          const SMikkTSpaceContext * pContext, const int iVertexRepresentitive)
{
	...
	for (face=0; face<iFaces; face++)
	{
		...
		if ( (pTriInfos[f].iFlag&GROUP_WITH_ANY)==0 )
		{
			...
			// project
			v1 = vsub(v1, vscale(vdot(n,v1),n)); if ( VNotZero(v1) ) v1 = MikkNormalize(v1);
			v2 = vsub(v2, vscale(vdot(n,v2),n)); if ( VNotZero(v2) ) v2 = MikkNormalize(v2);
	...
	// normalize
	if ( VNotZero(res.vOs) ) res.vOs = MikkNormalize(res.vOs);
	if ( VNotZero(res.vOt) ) res.vOt = MikkNormalize(res.vOt);
	if (fAngleSum>0)
	{
		res.fMagS /= fAngleSum;
		res.fMagT /= fAngleSum;
