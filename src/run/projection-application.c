...
typedef struct {
   ...
   Per_Frame per_frame STD140_ALIGN;
...
void projection_init(Projection_Application *app) {
...

   {
      ...
      set_framebuffer_draw_attachments(app->fb, 0b1);
...
void draw_scene_bunch_of_models(Projection_Application *app) {
...

   {
      ...
      {
         auto speed = (animation_paused ? 0.0 : 1.0);
         for (usz index = 0; index < count_of(scene_nodes); index += 1) {
            set_animation_speed(scene_nodes[index], speed);
         }
...
//////////////////////////////////////////////////////
void *generate_cone_map_relaxed_slower(const char *heightmap_path, const char *out_png_path, int *out_width, int *out_height) {
   trace_info("Generating relaxed cone map for `%s`, to be stored at `%s`", heightmap_path, out_png_path);
   ...
   ZString compute_shader_path = "res/shaders/conemap_generation_slower.glsl";
   ...
   for (int i = 0; i < width; i += 1) {
      for (int j = 0; j < height; j += 1) {
         ...
         upload_uniform_int  (compute_shader, "is_first_time", (i == 0 && j == 0) ? 1 : 0);
   ...
   // read back
   isz buffer_size = width * height * 4 * size_of(float);
   float *pixels = malloc(buffer_size);
...
bool generate_cone_map_relaxed(const char *heightmap_path, TString *out_generated_image_path, int *out_width, int *out_height) {
   auto out_png_path  = "conemap.png";
   auto out_cone_path = "conemap.exr";
   auto in_cone       = heightmap_path;
   {
      ...
      out_cone_path = path_create(base_directory, tprintf("%s.conemap.exr", base_name));
      out_png_path  = path_create(base_directory, tprintf("%s.conemap.png", base_name));
   }

   if (out_generated_image_path) *out_generated_image_path = out_png_path;

   Texture heightmap    = create_texture_from_filepath(heightmap_path);
   Texture cone_map_out = create_texture(heightmap.width, heightmap.height, nullptr, TEXTURE_FORMAT_RGBA32F, TEXTURE_TYPE_2D, TEXTURE_FILTER_NONE, TEXTURE_WRAP_REPEAT);
   Texture cone_map_in  = create_texture(heightmap.width, heightmap.height, nullptr, TEXTURE_FORMAT_RGBA32F, TEXTURE_TYPE_2D, TEXTURE_FILTER_NONE, TEXTURE_WRAP_REPEAT);
   memset_texture(cone_map_in, vector4(1.0), 0);

   int width = cone_map_out.width;
   int height = cone_map_out.height;

   ZString compute_shader_path = "res/shaders/conemap_generation.glsl";
   Shader  compute_shader = create_shader(compute_shader_path);

   if (!is_valid_shader(compute_shader)) {
      trace_error("%s: Failed to create shader `%s`", __func__, compute_shader_path);
      return false;
   }


   auto size = cone_map_out.width * cone_map_out.height * size_of(Vector2_Uint);
   Buffer xy_js = create_buffer(size);
   defer destroy_buffer(&xy_js);

   Vector2_Uint *data = malloc(size);
   defer free(data);
   // TODO: Generate the ivec2s based of the generation type ...
   // TODO: Keep everything very simple, the dispatch can be separate, but whenever possible we should condense and mergfe coommon parts between gfeneration modes




   {
      bind_buffer(&xy_js, BUFFER_TYPE_STORAGE, 0);
      bind_texture(heightmap, 0);
      bind_texture_as_image(cone_map_out, 0, TEXTURE_ACCESS_READ | TEXTURE_ACCESS_WRITE);
      bind_texture(cone_map_in, 1);
      bind_shader(compute_shader);
      upload_uniform_ivec2(compute_shader, "resolution", width, height);
   }
   ...
   const isz buffer_binding = 2;
   const int tile_size = 64;
   const int group = 16;
   enum { CONE_RADIUS, CONE_GEN_O2, CONE_GEN_ROW, CONE_GEN_TILE }
      // generation_mode = CONE_RADIUS;
   generation_mode = CONE_GEN_O2;
   upload_uniform_bool(compute_shader, "use_buffer", true);
   switch (generation_mode) {
      // One dispatch per tj (W*H)
      case CONE_GEN_O2: {
         trace_infof("Starting CONE_GEN_O2");
         for (int jy = 0; jy < height; jy += 1) {
            for (int jx = 0; jx < width; jx += 1) {
               // Update Storage Buffer
               data[0] = vector2_uint(jx, jy);
               update_buffer(&xy_js, data, 0, size_of(Vector2_Uint));
               bind_buffer(&xy_js, BUFFER_TYPE_STORAGE, 0);

               // Bind Storage Buffer
               const isz offset = 0;
               const isz size = size_of(Vector2_Uint);
               bind_buffer_view(&xy_js, BUFFER_TYPE_STORAGE, buffer_binding, offset, size);

               // Dispatch
               dispatch_compute_shader_2d(compute_shader, width, height, group);
               shader_memory_barrier(SHADER_BARRIER_IMAGE_ACCESS);
               // glMemoryBarrier(GL_ALL_BARRIER_BITS);

               // Swap textures
               Texture temp = cone_map_in;
               cone_map_in  = cone_map_out;
               cone_map_out = temp;
               bind_texture_as_image(cone_map_out, 0, TEXTURE_ACCESS_READ | TEXTURE_ACCESS_WRITE);
               bind_texture(cone_map_in, 1);
            ...
            glFinish();
            trace_info("o2 row %d/%d", jy + 1, height);
      ...
      case CONE_RADIUS: {
         upload_uniform_bool(compute_shader, "use_buffer", false);
         for (int passes = 0; passes < 1; passes += 1) {
            glFinish();
            dispatch_compute_shader_2d(compute_shader, width, height, group);
            shader_memory_barrier(SHADER_BARRIER_IMAGE_ACCESS);
         }
      ...
      default: {
         trace_fatal("You shall not pass");
      }
   }
   ...
   { // Read back and write to filesystem

      // glFinish();
      trace_info("%s About to read back to write cone png", __func__);

      const int channels = 4;
      const isz buffer_size = width * height * channels * size_of(float);
      float *pixels = malloc(buffer_size);
      glBindTexture(GL_TEXTURE_2D, cone_map_out.handle);
      glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_FLOAT, pixels);
      ...
      stbi_flip_vertically_on_write(true);
      stbi_write_png(out_png_path, width, height, 4, png_data, width * 4);
      trace_info("%s: Wrote %dx%d to `%s`", __func__, width, height, out_png_path);
      free(png_data);

      if (exr_write_image(out_cone_path, pixels, width, height, 4)) {
         trace_info("Wrote with great success");
      }
   ...
   if (out_width)  *out_width  = cone_map_out.width;
   if (out_height) *out_height = cone_map_out.height;
...
#include "./scenes/league.c"
#include "./scenes/depth.c"
...
void projection_update(Projection_Application *app, f64 dt) {
   ...
   const Vector2_Uint resolution = vector2_uint(app->fb.color.width, app->fb.color.height);
   ...
   // draw_scene_few2(app);
   // draw_scene_few(app);
   draw_scene_depth(app);
   // draw_scene_bunch_of_models(app);
   ...
   Texture ambient_occlusion_texture = {0};
   Framebuffer geometry_framebuffer = draw_indirect(app->fb, app->shader, &ambient_occlusion_texture);
   ...
   if (!is_window_minimized()) {
      if (false) {
         blit_framebuffer_to_swapchain(geometry_framebuffer);
         return;
      }
      ...
      if (please_bloom) {
         ldr_fb = apply_postprocess(bloomed, geometry_framebuffer, ambient_occlusion_texture);
      } else {
         ldr_fb = apply_postprocess(src_fb, geometry_framebuffer, ambient_occlusion_texture);
      }
