...
void main() {
   ...
   if (pixelCoord.x >= imageSize(outputImage).x
      || pixelCoord.y >= imageSize(outputImage).y)
   {
       return;
   }
...
void mainImage(out vec4 c_out, in vec2 f) {
   t = iTime * .125;
   vec3 col = vec3(0., 0., 0.);
   vec2 R = iResolution.xy, uv = vec2(f - R / 2.) / R.y;
   vec3 dir = camera(uv);
...
float scene(vec3 p) {
   ...
   vec3 op = p;
#ifdef FUNKY
   var = step(-1. + cos(floor(p.z * 6.) / 6. + iTime * 1.) * 3.14, mod(atan(p.x, p.y), 6.28) - 3.14) * step(mod(atan(p.x, p.y), 6.28) - 3.14 - 1.5, -1. + cos(floor(p.z * 3.) / 1. + iTime * 1.) * 3.14)

         * step(.0, (length(fract(vec2(op.z, min(abs(op.x), abs(op.y))) * 10.) - .5) - .2))

       ;
#else
   var = atan(p.x, p.y) * 1. + 0.;
#endif
