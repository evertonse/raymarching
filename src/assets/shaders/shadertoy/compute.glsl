...
float scene(vec3 p) {
   ...
   vec3 op = p;
#ifdef FUNKY
   var = step(-1. + cos(floor(p.z * 6.) / 6. + t * 1.) * 3.14, mod(atan(p.x, p.y), 6.28) - 3.14) * step(mod(atan(p.x, p.y), 6.28) - 3.14 - 1.5, -1. + cos(floor(p.z * 3.) / 1. + t * 1.) * 3.14)
...
#endif
   var = cos(var * 1. + floor(p.z) + t * (mod(floor(p.z), 2.) - 1. == 0. ? -1. : 1.));
