...
// Quad to Quad transform, it's where them
vec3 cameraDir(vec2 uv, Quad src, Quad dst) {
   vec3 res = vec3(0);
   return res;
}
...
void render(out vec4 color, vec3 camera_pos, vec3 camera_dir) {
   ...
   vec3 light_pos = vec3(10, 9, 2);
   vec3 light_dir = normalize(light_pos - intersection);


   vec3 light_pos2 = vec3(-10, 9, 2);
   vec3 light_dir2 = normalize(light_pos2 - intersection);

   vec3 normal = normalScene(intersection);
   float diffuse = dot(light_dir, normal)/2.0 + dot(light_dir2, normal)/2.0;
   float spec = 0;
   if (d < FAR) {
      // spec = brdf(camera_dir, -light_dir, normal).x;
      spec = brdf(normalize(intersection + camera_pos), normalize(intersection + light_pos), normal).x/2 + brdf(normalize(intersection + camera_pos), normalize(intersection + light_pos2), normal).x/2;
   }
   ...
   float shadow_result =
        softshadow(intersection + intersection_offset, light_dir, NEAR/2, FAR, 64)/2
      + softshadow(intersection + intersection_offset, light_dir2, NEAR/2, FAR, 32)/2.;
