...
#define Table_Header(K, V) \
   struct { \
      size_t cap, size; \
      struct { K key; V val; uint64_t hash; } *entries; \
   }
...
static inline size_t find(void *table, const void *key, size_t key_size, size_t entry_size, uint64_t key_hash) {
   ...
   for (size_t i = 0, pos = key_hash % t->cap; i < t->cap; i++, pos = (key_hash + i * i) % t->cap) {
      ...
      if (!*entry_hash) return pos; // empty slot
...
#define set(t, k, v) do { \
   ...
   if (i < (t)->cap) { \
      if (!(t)->entries[i].hash) (t)->size++; \
      (t)->entries[i].key = k; (t)->entries[i].val = v; (t)->entries[i].hash = h; \
   } \
} while (0)
...
static inline void* table_get_entry(void *table, const void *key, size_t key_size, size_t entry_size, uint64_t key_hash) {
   ...
   if (i < t->cap) {
      ...
      if (*entry_hash) return (uint8_t *)t->entries + i * entry_size + key_size;
   }
   return NULL;
}

#define get(t, k) \
   ((typeof(&(t)->entries[0].val)) \
    table_get_entry(t, \
                   _Generic(k, ConstCharPtr: k, char*: k, default: &k), \
                   _Generic(k, ConstCharPtr: strlen(k), char*: strlen(k), default: sizeof(k)), \
                   sizeof(*(t)->entries), \
                   hash_data(_Generic(k, ConstCharPtr: k, char*: k, default: &k), \
                            _Generic(k, ConstCharPtr: strlen(k), char*: strlen(k), default: sizeof(k)))))
