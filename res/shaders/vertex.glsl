...
layout (location = 0) out #include "./src/pipe.glsl" out_vertex;
...
void main() {
   ...
   // https://computergraphics.stackexchange.com/questions/1502/why-is-the-transposed-inverse-of-the-model-view-matrix-used-to-transform-the-nor/1506#1506
   ...
   {  // Send to next shader
      ...
      // NOTE: Is not specified then it's in *World Space*

      out_vertex.position       = position.xyz;
      out_vertex.normal         = normal;
      out_vertex.bitangent      = bitangent;
      out_vertex.tangent        = tangent.xyz;
      out_vertex.tangent_w_sign = tangent.w;
      out_vertex.has_tangents    = uint(draw_command.has_tangents);
      ...
      out_vertex.render_state            = floatBitsToUint(push_constants[0].x);
      out_vertex.color_tint              = instances[gl_BaseInstance + gl_InstanceID].color_tint;
      out_vertex.instance_rendering_mode = instances[gl_BaseInstance + gl_InstanceID].instance_rendering_mode;
      out_vertex.custom_1                = instances[gl_BaseInstance + gl_InstanceID].custom_1;
      out_vertex.custom_2                = instances[gl_BaseInstance + gl_InstanceID].custom_2;

      mat3 TTBN = transpose(mat3(tangent.xyz, bitangent, normal));
      out_vertex.TangentLightPosition = vec3(0);
      out_vertex.TangentViewPosition = TTBN * camera_position;
      out_vertex.TangentFragPosition = TTBN * out_vertex.position;
